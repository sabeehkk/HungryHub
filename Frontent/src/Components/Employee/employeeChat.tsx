...
function EmployeeChat() {
  ...
  const [checkChat,setCHeckChat] = useState(false)
    ...
    useEffect(() => {
      ...
      userAxios.get(`/getchat?id=${id}`).then((res)=>{
       console.log(res.data,'incoming datas');
       setUserdetails(res.data.findChat[0].employeeId)
   ...
   console.log(chatPerson,'fdsgadsgdfads')
    const handleMessage = () => {
       ...
       userAxios.post(`/saveChat`,{chat,orderId:location.state }).then((response) => {
                ...
                <a
                  ...
                  <div className="w-full pb-2">
                    <div className="flex justify-between">
                      <span className="block ml-2 font-semibold text-gray-600">{Userdetails.name}</span>
          ...
          <div className="hidden lg:col-span-2 lg:block">
            <div className="w-full">
              <div className="relative flex items-center p-1 border-b border-gray-300">
                <img className="object-cover w-10 h-10 rounded-full"
                  src="https://cdn.pixabay.com/photo/2018/09/12/12/14/man-3672010__340.jpg" alt="username" />
                <span className="block ml-2 font-bold text-gray-600">{Userdetails.name}</span>
